...
SELECT name, phone_number, passport_number FROM
    ...
    AS z ON people.id = z.person_id) AS x JOIN (SELECT license_plate FROM bakery_security_logs WHERE year = 2021 AND month = 7 AND day = 28 AND hour = 10 AND minute <= 25 AND minute >= 15 AND activity = "exit")
AS y ON x.license_plate = y.license_plate;